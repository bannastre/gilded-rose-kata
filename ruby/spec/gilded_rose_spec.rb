...
describe GildedRose do
  ...
  describe "#Aged Brie" do
    ...
    it "increases quality by 1 if sell_in > 0" do
      items = [AgedBrie.new("Aged Brie", 1, 0)]
      GildedRose.new(items).update_quality()
      expect(items[0].quality).to eq 1
    end

    it 'caps quality at 50' do
      items = [AgedBrie.new("Aged Brie", 0, 50)]
      GildedRose.new(items).update_quality()
  ...
  describe "#Backstage passes to a TAFKAL80ETC concert" do
    ...
    it "increases quality by 2 if sell_in > 5" do
      items = [BackStagePass.new("Backstage passes to a TAFKAL80ETC concert", 6, 0)]
      GildedRose.new(items).update_quality()
      expect(items[0].quality).to eq 2
    end
  ...
  describe "#Sulfuras, Hand of Ragnaros" do
    it "does not change quality if sell_in == 0" do
      items = [Sulfuras.new("Sulfuras, Hand of Ragnaros", 0, 0)]
      GildedRose.new(items).update_quality()
      expect(items[0].quality).to eq 0
    end

    it "does not change quality if sell_in > 0" do
      items = [Sulfuras.new("Sulfuras, Hand of Ragnaros", 1, 0)]
      GildedRose.new(items).update_quality()
      expect(items[0].quality).to eq 0
    end
  ...
  describe "#conjoured_item" do
    it "does not change the name" do
      # items = [Item.new("foo", 0, 0)]
      items = [ConjouredItem.new('foo', 1, 50)]
      GildedRose.new(items).update_quality()
      expect(items[0].name).to eq "foo"
    end
    ...
    it "reduces quality by 2 if sell_in == 0" do
      # items = [Item.new("foo", 0, 50)]
      items = [ConjouredItem.new('foo', 0, 50)]
      GildedRose.new(items).update_quality()
    ...
    it 'does not reduce quality below 0' do
      # items = [Item.new("foo", 0, 1)]
      items = [ConjouredItem.new('foo', 0, 1)]
      GildedRose.new(items).update_quality()
      GildedRose.new(items).update_quality()
      expect(items[0].quality).to eq 0
    end
    ...
    it "reduces sell_in by 1" do
      # items = [Item.new("foo", 1, 0)]
      items = [ConjouredItem.new('foo', 1, 0)]
      GildedRose.new(items).update_quality()
      expect(items[0].sell_in).to eq 0
    end
  end
